...
namespace NFCAndroidExample
{
    ...
    public class MainActivity : AppCompatActivity
    {
        ...
        protected override void OnNewIntent(Intent intent)
        {
            ...
            var tagType = Convert.ToString(myTag);
            var tagIdBytes = myTag.GetId();
            var tagIdString = ByteArrayToString(tagIdBytes); //Byte array converted to string
            var reverseHex="";


           if (tagType == "TAG: Tech [android.nfc.tech.IsoDep, android.nfc.tech.NfcA]") {
                reverseHex = LittleEndian32(tagIdString); // Reversed hex converted to hex
            ...
            if (tagType != "TAG: Tech [android.nfc.tech.IsoDep, android.nfc.tech.NfcA]")
            {
                reverseHex = LittleEndian64(tagIdString); // Reversed hex converted to hex
        ...
        /// <returns></returns>
        private static string LittleEndian32(string num)
        {
            var number = Convert.ToInt32(num, 16);
            //FEDE------->var number = Convert.ToUInt64(num, 16);

            var bytes = BitConverter.GetBytes(number);
            return bytes.Aggregate("", (current, b) => current + b.ToString("X2"));
        }
        private static string LittleEndian64(string num)
        {
            //FEDE------->var number = Convert.ToInt32(num, 16);
            var number = Convert.ToUInt64(num, 16);
        ...
        {
            var shb = BitConverter.ToString(ba).Replace("-", "");
            return shb.ToString();
        }
